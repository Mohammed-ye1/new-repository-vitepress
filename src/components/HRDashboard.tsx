...
export function HRDashboard({ entries, employees, isHR }: Props) {
  ...
  // Get unique departments and sections
  const uniqueDepartments = Array.from(new Set(Object.values(employees).map(emp => emp.department)));
  ...
  const uniqueEmployees = Array.from(new Set(entries.map(entry => entry.employeeId)))
    .map(id => employees[id])
    .filter(Boolean);

  const exportData = () => {
    const csvContent = [
      ['Date', 'Employee ID', 'Employee Name', 'Department', 'Section', 'Shift Type', 'Remark'],
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 lg:grid-cols-5 gap-4 mb-6">
          ...
          <div>
            ...
            >
              ...
              {shiftTypes.map((type) => (
                <option key={`shift-${type}`} value={type}>
