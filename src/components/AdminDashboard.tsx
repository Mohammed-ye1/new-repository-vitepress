...
export function AdminDashboard({ 
  ...
  const handleDelete = async (employeeId: string) => {
    try {
      ...
      const tempSuccess = `Employee ${employeeId} and all associated records have been deleted successfully`;
      setSuccess(tempSuccess);
      setTimeout(() => setSuccess(''), 3000);
    } catch (err) {
      console.error('Error deleting employee:', err);
      setError('Failed to delete employee and associated records. Please try again.');
